...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        mainPanel(
            plotOutput("heatMap",width="110%",height=400)
        )
    )
)
...
#server <- function(input, output) 
{
    ...
    aspect_ratio<-2.5  
    height<-7
    ggplot(data=state_pcb())+geom_polygon(aes(x=long,y=lat,fill=Branches_Per_Capita,group=group),color="white")+theme_classic()+xlab("Longitude")+ylab("Latitude")+labs(title=paste("State-wise Bank Branch Density Per 10000 people in ",input$year))+labs(fill="Branch Density")+scale_fill_gradient(low="white",high="#CB454A")
    })
}
)
